...
class Bmr:
    ...
    def calculateTempDifference(self, temperature):
        return int(temperature) - 18

    @authenticated
    def setManualTemp(self, room_id, temperature):
        headers = {"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}
        temp_diff = self.calculateTempDifference(temperature)
        data = {"manualTemp": f"{str(room_id).zfill(2)}0{str(temp_diff).zfill(2)}0"}
        response = self._http.post("/saveManualTemp", headers=headers, data=data)
        if response.status_code !=200:
